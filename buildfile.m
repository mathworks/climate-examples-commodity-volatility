...
%buildfile Build file for the Commodity Volatility Modelling toolbox.
...
% Add a task to export the Live Scripts as PDFs.
...
scripts = ["S01_DataProcessing", "S02_TrendDecomposition", ...
    "S03_NonParametricVolatility", "S04_ModelBasedVolatility"];
mainFolder = fullfile( root, "tbx", tbxname() );
docIn = fullfile( mainFolder, scripts + ".m" );
docOut = fullfile( mainFolder, scripts + ".pdf" );

% Export to PDF.
for fileIdx = 1 : numel( docIn )
    export( docIn(fileIdx), docOut(fileIdx), ...
        "Format", "pdf", ...
        "Run", true );
end % for
